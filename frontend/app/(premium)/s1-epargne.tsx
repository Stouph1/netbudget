// S1 — Épargne & Placements (premier écran Grand Dispatching Premium).
//
// Affiche :
//  - Header total : somme des objectifs (hors ExtraP) + % progression global
//  - Liste des objectifs avec barre de progression individuelle
//  - Bouton "+" flottant → modal add/edit
//
// Stockage : encrypted_payloads (via premiumStore.saveS1/loadS1). Encryption
// stub pour Phase 3 — sera remplacé par libsodium en Phase 5.
...
// Palette (voir design-system/netbudget-premium/MASTER.md)
...
export default function S1Epargne() {
  ...
  const upsertGoal = useCallback(
    (goal: SavingsGoal) => {
      const idx = payload.goals.findIndex((g) => g.id === goal.id);
      const next = { ...payload };
      if (idx >= 0) {
        next.goals = payload.goals.map((g) => (g.id === goal.id ? goal : g));
      } else {
        next.goals = [...payload.goals, goal];
      }
      persist(next);
    },
    [payload, persist],
  ...
  const deleteGoal = useCallback(
    (id: string) => {
      const next = { ...payload, goals: payload.goals.filter((g) => g.id !== id) };
      persist(next);
    },
    [payload, persist],
  );
    ...
    <SafeAreaView style={styles.safe} edges={["top", "left", "right"]}>
      ...
      <View style={styles.totalCard}>
        ...
        <Text style={styles.totalMeta}>
          {totals.count} objectif{totals.count > 1 ? "s" : ""} · {totals.pct.toFixed(0)}%
        </Text>
      </View>
